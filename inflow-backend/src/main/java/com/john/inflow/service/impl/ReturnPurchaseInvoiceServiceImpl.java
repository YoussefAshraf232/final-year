...
public class ReturnPurchaseInvoiceServiceImpl implements ReturnPurchaseInvoiceService {
    ...
    public ReturnPurchaseInvoiceResponse create(CreateReturnPurchaseInvoiceRequest request, Integer userId) {
        ...
        for (ReturnPurchaseInvoiceItemRequest itemRequest : request.items()) {
            ...
            stockMovementService.record(product, warehouse, "PURCHASE_RETURN", -itemRequest.amount(), itemRequest.priceAtReturn(), "RETURN_PURCHASE_INVOICE", savedInvoice.getId(), null, user);
        ...
        auditLogService.log(user, "CREATE", "RETURN_PURCHASE_INVOICE", savedInvoice.getId(), "totalPrice=" + totalPrice);
